...
export type PaymentReason =
  ...
  // M2: card money flows
  | "cardCreation" | "loanPrincipal" | "loanInterest" | "loanRepayment";

/** Serializable snapshot of a card instance, stored in the event log. */
export interface CardSnapshot {
  id: string;
  kind: "loan" | "insurance" | "corporation" | "rebate";
  owner: Account;
  subject: PlayerId;
  state: unknown;
}
...
export type GameEvent =
  ...
  // M2: financial-instrument card events
  | { type: "CardCreated"; card: CardSnapshot }
  | { type: "InterestCharged"; loanId: string; amount: number }
  | { type: "LoanRepaid"; loanId: string; amount: number; remaining: number }
  | { type: "CardVoided"; cardId: string; reason: "bankruptcy" }
