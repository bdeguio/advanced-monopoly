...
export interface PlayerState {
  ...
  pos: number;
  inJail: boolean;
  jailAttempts: number;   // failed doubles attempts while in jail
...
export interface AuctionState {
  tile: number;
  active: PlayerId[];     // players still in the auction, in bid order
  turnPtr: number;        // index into active[] of whose bid it is
  highBid: number;
  highBidder?: PlayerId;
}
...
export interface GameState {
  ...
  houseSupply: number;
  hotelSupply: number;
  /** Deck orders fixed at game start; cursors advance modulo length. */
  chanceOrder: number[];
  chestOrder: number[];
  chanceCursor: number;
  chestCursor: number;
  /** GOJF cards currently held by players (removed from circulation). */
  jailCardsOut: { chance: boolean; chest: boolean };
  diceCursor: number;     // RNG cursor for the main dice stream
  utilCursor: number;     // RNG cursor for utility-card rolls
  lastRoll?: { d1: number; d2: number };
  auction?: AuctionState;
  /** FIFO queue of unmet obligations; head is settled first. */
  debts: DebtState[];
  /** M2: live financial-instrument cards, in creation order. */
  cards: Card[];
  /** M2: monotonic counter for deterministic card ids. */
  cardSeq: number;
  /** Tile awaiting a Buy/Decline decision by the current player. */
  pendingPurchase?: number;
  /** Phase to restore once all debts are settled. */
  resumePhase?: Phase;
  winner?: PlayerId;
}
...
/** Look at deck card by absolute cursor position. */
...
export function reduce(s: GameState | undefined, e: GameEvent): GameState {
  ...
  switch (e.type) {
    ...
    case "UtilityRolled":
      ...
      break;
    case "SpeedingToJail":
      break; // informational; SentToJail follows
    case "TokenMoved":
    ...
    case "InterestCharged":
      // pure fact; the paired MoneyTransferred moves the money. No principal change.
      break;
    case "LoanRepaid": {
    ...
    case "PlayerBankrupted": {
      ...
      p.bankrupt = true;
      p.cash = 0;
      // return any held GOJF cards to circulation
      for (const deck of p.jailCards) n.jailCardsOut[deck] = false;
      p.jailCards = [];
      n.debts = n.debts.filter((d) => d.debtor !== e.player);
      // M2: void loans where this player was the borrower (creditor claimed the estate)
...
export function reduce(s: GameState | undefined, e: GameEvent): GameState {
  ...
  switch (e.type) {
    ...
    case "AuctionAbandoned":
      ...
      break;
    case "CardDrawn": {
      // Advance the cursor just past the drawn card (skipped GOJF cards stay skipped).
      const order = e.deck === "chance" ? n.chanceOrder : n.chestOrder;
      const defs = e.deck === "chance" ? CHANCE_DECK : CHEST_DECK;
      let c = e.deck === "chance" ? n.chanceCursor : n.chestCursor;
      let guard = 0;
    ...
    case "SentToJail": {
      ...
      p.inJail = true;
      p.jailAttempts = 0;
      p.doublesCount = 0; // no bonus roll after being jailed
      break;
    }
    case "LeftJail": {
      const p = n.players[e.player];
    ...
    case "HouseBuilt": {
      ...
      if (prop.houses === 5) {
        n.hotelSupply -= 1;
        n.houseSupply += 4; // four houses return to the bank
      } else {
        n.houseSupply -= 1;
      }
      break;
    ...
    case "HouseSold": {
      ...
      if (prop.houses === 5) {
        n.hotelSupply += 1;
        n.houseSupply -= 4; // hotel breaks back into four houses
      } else {
        n.houseSupply += 1;
      }
      prop.houses -= 1;
